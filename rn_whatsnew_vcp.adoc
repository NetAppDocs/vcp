...
[.lead]
NetApp Element Plug-in for vCenter Server 5.2 provides important supportability and security improvements and an enhancement to the reporting for custom Protection Domains. With version 5.2, the plug-in now displays Custom Protection Domain Health information when a custom Protection Domain is configured on an Element cluster. link:vcp_task_reports_overview.html#reporting-overview-page-data[Learn more].
