...
Performing remote replication involves these tasks:
...
. Click the *Cluster Pairs* sub-tab.
. Click *Create Cluster Pairing*.
...
For instructions on performing cluster pairing from the Element web UI, see
https://docs.netapp.com/sfe-122/topic/com.netapp.doc.sfe-ug/home.html[Managing storage with NetApp Element software^]
...
. Click the *Cluster Pairs* sub-tab.
. Click *Complete Cluster Pairing*.
+
...
. From the *Active* view, check the volume that you want to pair.
. Click *Actions*.
. Click *Volume Pairing*.
. Select one of the following:
...
.. *Optional*: Click the *Set remote volume to Replication Target* option if you want to set the remote volume as the target in the volume pairing. The local volume, if set to read/write, becomes the source in the pair.
+
...
.. Enter a volume ID.
.. Click the *Set remote volume to Replication Target* option if you want to set the remote volume as the target in the volume pairing. The local volume, if set to read/write, becomes the source in the pair.
+
IMPORTANT: If you assign an existing volume as the replication target, the data on that volume will be overwritten. As a best practice, you should use a new volume as the replication target.
+
NOTE: You can also assign replication source and target later in the pairing process from *Volumes* > *Actions* > *Edit*. You must assign a source and target to complete the pairing.
...
For instructions on starting or completing volume pairing from the Element web UI, see https://docs.netapp.com/sfe-122/topic/com.netapp.doc.sfe-ug/home.html[Managing storage with NetApp Element software^]
...
. Click the *Volumes* sub-tab.
. From the *Active* view, check the volume you want to pair.
. Click *Actions*.
. Click *Volume Pairing*.
...
=== Create target volumes and pairing them with local volumes
...
. Click the *Volumes* sub-tab.
. From the *Active* view, check two or more volumes that you want to pair.
. Click *Actions*.
. Click *Volume Pairing*.
. Select a *Replication Mode*:
+
...
. Click the *Volumes* sub-tab.
. From the *Active* view, check the volume that you want to edit.
. Click *Actions*.
. Click *Edit*.
...
. Select *NetApp Element Management* > *Management*.
. Click the *Volumes* sub-tab.
. From the *Active* view, check the volume you want to edit.
. Click *Actions*.
. Select *Edit*.
...
. Click the *Volume Pairs* sub-tab.
...
. Click the *Volume Pairs* sub-tab.
. Check the volume pair you want to edit.
. Click *Actions*.
. Select *Edit*.
...
For volumes not controlled by the vCenter Plug-in, you can alternatively delete a volume pair end using the Element web UI. For instructions on deleting a volume pair end from the Element web UI, see link:https://docs.netapp.com/sfe-122/topic/com.netapp.doc.sfe-ug/home.html[Managing storage with Element software].
...
. Click the *Volume Pairs* sub-tab.
. Check the one or more volume pairs you want to delete.
. Click *Actions*.
. Select *Delete*.
...
. (Optional for clusters managed by plug-in) Check *Change Replication Target Access to* and select a new access mode for the replication target volume. This new access mode will be applied after the volume pairing relationship has been removed.
. Click *Yes*.
...
For clusters not controlled by the vCenter Plug-in, you can alternatively delete a cluster pair end using the Element web UI. For instructions on deleting a cluster pair end using the Element web UI, see link:https://docs.netapp.com/sfe-122/topic/com.netapp.doc.sfe-ug/home.html[Managing storage with Element software].

.Steps
. Select *NetApp Element Management* > *Protection*.
. Click the *Cluster Pairs* sub-tab.
. Check the cluster pair you want to delete.
. Click *Actions*.
. Select *Delete*.
. Confirm action.
+
