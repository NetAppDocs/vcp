---
sidebar: sidebar
permalink: vcp_task_clusters_manage_5_0.html
...
* At least one cluster must be link:vcp_task_getstarted_5_0.html#add-storage-clusters-for-use-with-the-plug-in[added].
+
...
* Firewall rules enable open network communication between the vCenter and the cluster MVIP on TCP ports 443, 8333, and 8443.
...
== View cluster details
You can view cluster details from the NetApp Element Remote Plugin extension point.
...
== Edit a cluster profile
You can change the cluster User ID and password from the NetApp Element Remote Plugin extension point.
...
== Remove a cluster profile
You can remove the profile of a cluster that you no longer want to manage from the vCenter Plug-in using the NetApp Element Remote Plugin extension point.
...
NOTE: To use link:vcp_concept_linkedmode.html[vCenter Linked Mode], you must register the remote Element Plug-in with each vCenter Server that will be managing NetApp SolidFire storage clusters from a separate management node.
...
== Enable Encryption at Rest
You can manually enable encryption at rest (EAR) functionality using the NetApp Element Remote Plugin extension point.
...
== Disable Encryption at Rest
You can manually disable encryption at rest (EAR) functionality using the NetApp Element Remote Plugin extension point.
...
You can manually enable a Secure Shell (SSH) session using the NetApp Element Remote Plugin extension point. Enabling SSH allows NetApp technical support engineers access to storage nodes for troubleshooting for the duration you determine appear.
...
== Disable SSH
You can manually disable Secure Shell (SSH) access to nodes in the storage cluster using the NetApp Element Remote Plugin extension point.
...
== Set protection domain monitoring
You can manually enable link:vcp_concept_protection_domains.html[protection domain monitoring] using the NetApp Element Remote Plugin extension point. You can also select a protection domain threshold based on node or chassis domains.
...
After you have set monitoring preferences, you can monitor protection domains from the link:vcp_task_reports_overview.html#reporting-overview-page-data[Reporting] tab of the plug-in extension point.

== Shut down a cluster
You can manually shut down all active nodes in a storage cluster using the NetApp Element Remote Plugin extension point.

If you want to link:vcp_task_add_manage_nodes.html#restart-a-node[restart] rather than shut down the cluster, you can select all nodes from the Management > Cluster page and perform a restart.
...
== Expand your NetApp HCI infrastructure
You can manually expand your NetApp HCI infrastructure by adding nodes using NetApp HCI. A link to a NetApp HCI UI for scaling your system is provided from the NetApp Element Remote Plugin extension point. Additional links are provided within the plug-in from the Getting Started and Cluster pages.
