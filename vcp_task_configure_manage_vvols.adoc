...
You can create storage containers from the VVols tab in the vCenter Plug-in extension point. You must create at least one storage container to begin provisioning VVol-backed virtual machines.
...
. In your vSphere Web Client, open the *Clusters* tab:
+
* For Element Plug-in for vCenter Server 4.10 and earlier, select *NetApp Element Management > VVols*.
* Beginning with Element Plug-in for vCenter Server 5.0, select *NetApp Element Remote Plugin > Management > VVols*.
...
== Monitor virtual volume resources
You can review virtual volume component performance and settings from the vCenter Plug-in:
...
=== Monitoring storage containers
You can review general data for all active storage containers on the cluster or detailed data for each storage container. The vCenter Plug-in tracks storage container efficiency, performance, and associated virtual volumes.
...
== Delete a storage container
You can delete storage containers from the vCenter Plug-in extension point.
