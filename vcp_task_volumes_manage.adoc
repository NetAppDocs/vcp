...
summary: Using the NetApp Element Plug-in for vCenter Server, you can create, view, edit, delete, clone, backup or restore volumes for user accounts. You can also manage each volume on a cluster, and add or remove volumes in volume access groups.
...
You can accomplish the following volume management tasks:
...
* <<Apply a QoS policy to multiple volumes>>
...
* Restore the volume from Element
* Back up the volume to Element

.Steps to restore the volume from Element
...
.Steps to back up the volume to Element
. Select *NetApp Element Management > Management*.
. If two or more clusters are added, select the cluster in the navigation bar.
. Select the *Volumes* subtab.
. From the *Active* view, check the volume.
...
. If two or more clusters are added, select the cluster in the navigation bar.
. Select the *Volumes* subtab.
. Check each volume that you want to add to an access group.
. Select *Actions*.
...
. If two or more clusters are added, select the cluster in the navigation bar.
. Select the *Volumes* subtab.
. Check each volume that you want to remove from an access group.
. Select *Actions*.
...
. If two or more clusters are added, select the cluster in the navigation bar.
. Select the *Volumes* subtab.
. Check each volume to which you want to apply a QoS policy.
. Select *Actions*.
. Select *Apply QoS Policy*.
. Select the QoS policy to apply to the selected volumes.
. Select *OK*.
...
.What you'll need
The volume you want to modify must be associated with a QoS policy.

.Steps
. Select *NetApp Element Management > Management*.
. If two or more clusters are added, select the cluster in the navigation bar.
. Select the *Volumes* subtab.
. Check each volume that contains a QoS policy you want to modify.
. Select *Actions*.
. Select *Edit
. Under *Quality of Service*, select a new QoS policy or custom settings to apply to the selected volumes.
+
NOTE: You can also select *Reset Default QoS* to restore default IOPS values.
