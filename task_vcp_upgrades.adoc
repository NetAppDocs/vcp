...
* When you upgrade from Element Plug-in for VMware vCenter Server 4.x to 5.0, the clusters already configured with the plug-in are lost because the data cannot be copied from a vCenter instance on a local plug-in to a vCenter instance on a remote plug-in. You must re-add the clusters to the remote plug-in. This is on a one-time activity when upgrading from a local to a remote plug-in.
====
