...
+
*** *Protection Level*: Indicates the overall protection level status against the failure of one of the custom Protection Domains for the cluster.
*** *Block Capacity*: Indicates the current protection level status of the block services subsystem against the failure of one of the custom Protection Domains for the cluster.
+
It also indicates the free block capacity for which the block services subsystem can maintain the "Fault Resilient Protection level" status.

*** *Metadata Capacity*: Indicates the current protection level status of the metadata services subsystem against the failure of one of the custom Protection Domains for the cluster.
*** *Ensemble Nodes*: Indicates the current protection level status of the ensemble members subsystem against the failure of one of the custom Protection Domains for the cluster.
