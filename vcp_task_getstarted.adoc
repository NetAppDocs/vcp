...
= Install and configure the NetApp Element Plug-in for VMware vCenter Server
...
[.lead]
You can install the most recent version of the NetApp Element Plug-in for VMware vCenter Server directly to your vCenter and access the plug-in with the vSphere Web Client.
...
NOTE: Using the Element vCenter plug-in to manage cluster resources from other vCenter Servers using link:vcp_concept_linkedmode.html[vCenter Linked Mode] is limited to local storage clusters only.
...
* Using the Element vCenter plug-in to manage cluster resources from other vCenter Servers using link:vcp_concept_linkedmode.html[vCenter Linked Mode] is limited to local storage clusters only.
====
...
+
NOTE: See this {url-peak}[KB^] to troubleshoot if the status is any of the following:
