...
After successful installation or upgrade, the extension points appear in the Shortcuts tab of the vSphere Web Client in the side panel. The extension points differ depending on your Element Plug-in for vCenter Server version.
...
.About this task
This procedure describes how to add a cluster profile so that the cluster can be managed by the vCenter Plug-in. You cannot modify cluster administrator credentials using the plug-in.
